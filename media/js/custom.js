    ...
    jQuery(element).each(function()
    {
        ...
        currentlistitem.each(function()
        {
            if (!jQuery(this).find('ul').hasClass('opened')){
                jQuery(this).find('ul').addClass("closed").css({
                    display:"none"
                });
            }
        });
            ...
            jQuery( this ).bind ("click",function(){
                ...
                if(disable == false){
                    ...
                    jQuery(container_to_hide+":visible").fadeOut(duration,function(){
                        ...
                        jQuery(".list_"+i).fadeIn(duration, function(){
                            disable=false;
                        });
                    });
                }
...
function runEffect(element, effect) {
    //run the effect
    jQuery(element).toggle(effect,{},500);
}

function bRequest(data, form, options) {
    var b = jQuery('button', form);
    var s = jQuery('span', form);
    s.css('background-color', null);
    s.show().text('Bitte warten');
    b.attr('disabled', true);
}

function aRequest(response, status, x, f) {
    var b = jQuery('button', f);
    var s = jQuery('span', f);
    var backgroundColor = '#b8dd54';
    ...
    s.css('background-color', backgroundColor);
    s.text(response.message).fadeOut(fadeOutTime);
